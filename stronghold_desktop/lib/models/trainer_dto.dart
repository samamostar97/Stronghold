...
class PagedTrainersResult {
  final List<TrainerDTO> items;
  final int totalCount;
  final int pageNumber;
  final int pageSize;
  final int totalPages;

  const PagedTrainersResult({
    required this.items,
  ...
  factory PagedTrainersResult.fromJson(Map<String, dynamic> json) {
    final itemsList = (json['items'] as List<dynamic>?)
            ?.map((e) => TrainerDTO.fromJson(e as Map<String, dynamic>))
            .toList() ??
        <TrainerDTO>[];

    return PagedTrainersResult(
      items: itemsList,
