...
class GymVisitsApi {
  ...
  static Future<List<CurrentVisitorDTO>> getCurrentVisitors() async {
    ...
    final response = await http.get(
      ApiConfig.uri('/api/admin/gym-visits/current'),
      headers: {
  ...
  static Future<CurrentVisitorDTO> checkIn(int userId) async {
    ...
    final response = await http.post(
      ApiConfig.uri('/api/admin/gym-visits/check-in'),
      headers: {
        'Content-Type': 'application/json',
        if (token != null) 'Authorization': 'Bearer $token',
      },
  ...
  static Future<void> checkOut(int visitId) async {
    ...
    final response = await http.post(
      ApiConfig.uri('/api/admin/gym-visits/$visitId/check-out'),
      headers: {
        'Content-Type': 'application/json',
        if (token != null) 'Authorization': 'Bearer $token',
      },
    ...
    if (response.statusCode == 204) {
      ...
      final error = json.decode(response.body);
      throw Exception(error['detail'] ?? 'Check-out failed');
    } else {
